...
export default function RoomDetail({ route }) {
    ...
    if (!isConnected) {
      ...
      stomp.connect({ Authorization: `Bearer ${SecureStore.getItem('token')}` }, () => {
        ...
        stomp.subscribe(`/topic/room/${route.params.roomId}/end`, (response) => {
          navigation.navigate(`RoomResult`, { roomId: route.params.roomId, questionId: route.params.questionId });
        });
      });
  ...
  const getQuestions = async () => {
    try {
      const { data } = await apiClient.get(`/rooms/${route.params.roomId}/question/${route.params.questionId}`, {
        headers: {
      ...
      <View>
        {
          question?.choices?.map((choice, index) => (
            <Text key={index} onPress={() => handleAnswerQuestion(choice?.id)}>{choice.choiceText}</Text>
          ))
        }
      </View>
    </View>
